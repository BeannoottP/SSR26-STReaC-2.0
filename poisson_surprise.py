...
def run_poisson_surprise(
    spike_times, min_spikes=3, max_spikes=10, surprise_threshold=3.0
):
    """
    ...
    \t surprise_threshold=3.0 (float): Threshold for the surprise value to consider a burst.

    Returns
    --------
    \t bursts (list of tuples): List of tuples in form ([surprise, burst_spike_times]).
    """
...
def burst_properties(bursts):
    """
    Calculate burst properties from detected bursts.
    ...
    Returns
    -------
    \t burst_props (list): List of burst properties.
    """
    ...
    inter_burst_intervals = np.zeros(np.max([n_bursts - 1, 0]))
